...
def create_secured_socket():
    ...
    # Wrap the socket with TLS (Transport Layer Security)
    ssl_context = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
    ssl_context.options |= ssl.OP_NO_TLSv1  # Disable insecure TLS versions
    ssl_context.options |= ssl.OP_NO_TLSv1_1  # Disable insecure TLS versions

    # Load a certificate and private key (replace with your own certificate files)
